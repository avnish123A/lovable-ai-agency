...
async function generateRichAIContent(
  ...
  description: string
): Promise<{ ai_description: string; ai_benefits: string; ai_eligibility: string; ai_terms: string }> {
  const empty = { ai_description: "", ai_benefits: "", ai_eligibility: "", ai_terms: "" };
  try {
    const res = await fetch(AI_GATEWAY, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a fintech copywriter for Indian users. Generate structured content for a financial product. Return EXACTLY this format with these section headers:

DESCRIPTION:
(2-3 sentence compelling summary under 60 words)

BENEFITS:
• (list 4-5 key benefits, one per line with bullet)

ELIGIBILITY:
• (list 3-4 eligibility requirements like age, income, documents)

TERMS:
• (list 3-4 simplified terms and conditions including fees, charges, validity)

Do not use markdown. Keep language simple and clear.`,
          },
          {
            role: "user",
    ...
    if (res.status === 429 || res.status === 402) {
      console.log(`AI rate/payment limited [${res.status}], skipping`);
      return empty;
    }
    if (!res.ok) {
    ...
    // Parse sections
    const descMatch = content.match(/DESCRIPTION:\s*([\s\S]*?)(?=BENEFITS:|$)/i);
    const benefitsMatch = content.match(/BENEFITS:\s*([\s\S]*?)(?=ELIGIBILITY:|$)/i);
    const eligibilityMatch = content.match(/ELIGIBILITY:\s*([\s\S]*?)(?=TERMS:|$)/i);
    const termsMatch = content.match(/TERMS:\s*([\s\S]*?)$/i);

    return {
      ai_description: descMatch?.[1]?.trim() || content.slice(0, 200),
      ai_benefits: benefitsMatch?.[1]?.trim() || "",
      ai_eligibility: eligibilityMatch?.[1]?.trim() || "",
      ai_terms: termsMatch?.[1]?.trim() || "",
    };
  } catch (e) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const deal of allDeals) {
      ...
      if (existing) {
        await supabase.from("finance_deals").update(deal).eq("deal_id", deal.deal_id);
        updated++;
      } else {
    ...
    // Generate rich AI content for deals missing it (max 8 per sync)
    let aiGenerated = 0;
    if (lovableApiKey) {
      ...
      for (const deal of missingAI || []) {
        const aiContent = await generateRichAIContent(
          lovableApiKey,
          deal.title,
          deal.merchant,
