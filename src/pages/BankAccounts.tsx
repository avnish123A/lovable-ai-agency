...
import { Star, ExternalLink, Landmark } from "lucide-react";
      ...
      <section className="pt-28 pb-20">
        <div className="container mx-auto px-4 md:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-heading font-bold mb-4">Best <span className="text-gradient">Bank Accounts</span></h1>
            <p className="text-muted-foreground max-w-xl mx-auto">Compare savings, current & salary accounts from India's top banks</p>
          </motion.div>
                    ...
                    <CardContent className="p-6 space-y-4">
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <h3 className="text-lg font-heading font-bold text-foreground">{p.account_name}</h3>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        {p.min_balance && <div><p className="text-muted-foreground text-xs">Min Balance</p><p className="font-semibold text-foreground">{p.min_balance}</p></div>}
                        {p.interest_rate && <div><p className="text-muted-foreground text-xs">Interest Rate</p><p className="font-semibold text-foreground">{p.interest_rate}%</p></div>}
                      </div>
                      {p.features?.length > 0 && (
                        <ul className="space-y-1">
                          {p.features.slice(0, 3).map((f: string, idx: number) => (
                            <li key={idx} className="text-xs text-muted-foreground flex items-start gap-1.5"><Landmark className="w-3 h-3 text-primary mt-0.5 shrink-0" />{f}</li>
                          ))}
                        </ul>
                      )}
                      {p.apply_link && (
