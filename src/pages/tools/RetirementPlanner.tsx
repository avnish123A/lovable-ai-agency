...
const RetirementPlanner = () => {
  ...
  const result = useMemo(() => {
    ...
    const chartData: { year: number; corpus: number; needed: number }[] = [];
    for (let y = 0; y <= years; y += Math.max(1, Math.floor(years / 20))) {
      ...
      chartData.push({ year: age + y, corpus: Math.round(fvE + fvS), needed: Math.round(neededAtY) });
    }
  ...
  const aiInsights = useMemo(() => getRetirementInsights(age, retireAge, monthly, result.corpus, result.needed, result.readiness), [age, retireAge, monthly, result]);
    ...
    <ToolLayout title="Retirement Planner" description="Plan your retirement corpus with real market benchmarks" icon={<Landmark className="w-7 h-7 text-primary" />}>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-4">
          <EditableSliderInput label="Current Age" value={age} onChange={setAge} min={18} max={55} step={1} suffix=" yrs" />
          <EditableSliderInput label="Retirement Age" value={retireAge} onChange={setRetireAge} min={45} max={70} step={1} suffix=" yrs" />
          <EditableSliderInput label="Monthly SIP" value={monthly} onChange={setMonthly} min={1000} max={200000} step={1000} prefix="₹" />
          <EditableSliderInput label="Annual SIP Step-up" value={stepUpRate} onChange={setStepUpRate} min={0} max={25} step={1} suffix="%" />
          <EditableSliderInput label="Existing Savings" value={existing} onChange={setExisting} min={0} max={10000000} step={50000} prefix="₹" />
          <EditableSliderInput label={`Expected Return (Nifty 50 avg: ${INDIAN_BENCHMARKS.sipReturns.nifty50_10yr}%)`} value={returnRate} onChange={setReturnRate} min={6} max={18} step={0.5} suffix="%" />
          <EditableSliderInput label={`Inflation Rate (CPI: ${INDIAN_BENCHMARKS.cpiInflation}%)`} value={inflation} onChange={setInflation} min={3} max={10} step={0.5} suffix="%" />
